...
const AllCategory = ({id, name, price, imgUrl }) => {
  ...
  const  quntaty =gitItemqunatity(id);
  return (
               ...
               <div className="product">
                 {/* {AllCategorys.map((item)=>( */}
                   <div className="Cards">
                     ...
                     <span className='name' style={{fontFamily:'Lobster'}}>{name}</span>
                     <span className='price'>{formatCurrency(price)}</span>
                     </div>
                     <div className='add-to-cart'>
                      {quntaty === 0 ? (<button className="CartBtn" onClick={()=>increaseCartQunatity(id )}>
                               ...
                               <div>
                                 <div className='incrce'>
                                      <button onClick={()=>decreaseCartQunatity(id)} className='btn'>-</button>
                                      <span className='quntit'>{quntaty} item i cart</span>
                                      <button onClick={()=>increaseCartQunatity(id)} className='btn'>+</button>
